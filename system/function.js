...
/* load file */
export async function loads(filename) {
    // Jalankan bash system/run.sh <filename> dan ambil hasil stdout
    const { stdout } = await execAsync(`bash system/run.sh ${filename}`);
    // Parse hasil stdout sebagai JSON
    const parsed = JSON.parse(stdout);
    let code = parsed.data;
    code = code.replace(/export default /, 'module.exports = ');
    code = code.replace(/export /g, 'exports.');
    // Siapkan objek module dan exports
    const moduleObj = { exports: {} };
    // Buat fungsi baru dari kode yang sudah diubah
    const fn = new Function('module', 'exports', code);
    // Jalankan fungsi untuk mengisi module.exports
    fn(moduleObj, moduleObj.exports);
    // Kembalikan hasil exports
