import { isGroupJid, isLidJid, normalizeRecipientJid } from 'zapo-js'
...
export async function resolveLid(client, jid) {
  ...
  if (typeof resolver === 'function') {
    try {
      ...
      if (pair?.lidJid) return normalizeJid(pair.lidJid)
    } catch {
  ...
  if (contactStore?.getByPhoneNumber) {
    try {
      ...
      if (contact?.lid) return normalizeJid(contact.lid)
      if (contact?.jid && isLidJid(contact.jid)) return normalizeJid(contact.jid)
    } catch {
