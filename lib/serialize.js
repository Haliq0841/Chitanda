...
export async function msg(sock, msg, db) {
    ...
    m.isOwner = m.sender && db?.data?.setting?.owner.map(v => v.replace(/[^0-9]/g, '') + "@s.whatsapp.net").includes(m.sender)
    if (m.message) {
    ...
    m.reply = async (text, trs, options = {}) => {
        const ms = typeof text === "string" && trs ? await func.tr(text, db.data.setting.lang) : text
