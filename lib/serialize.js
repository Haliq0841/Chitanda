...
import { resolveDatabaseJid, resolveLid, resolveOwnerJids } from "./identity.js"
...
export function Client(db, options = {}) {
    ...
    const sanitizeMessagePayload = (payload) => {
        ...
        if (payload && typeof payload === 'object') {
            const next = { ...payload }
            if (next.botForwardedMessage || next.richResponseMessage) return next
...
export async function msg(sock, msg, db) {
    ...
    if (msg.key) {
        m.key = msg.key
        const pickLidFirst = (...values) => {
            const candidates = values.filter(Boolean).map(value => jidNormalizedUser(value))
            return candidates.find(value => value.endsWith('@lid')) || candidates[0] || ''
        }
        const remoteJid = pickLidFirst(m.key.remoteJid, m.key.remoteJidAlt)
        const participant = pickLidFirst(m.key.participant, m.key.participantAlt, msg.participant)
        ...
        m.sender = await resolveLid(sock, senderIdentity)
    ...
    if (m.isGroup) {
        ...
        const participants = Array.isArray(groupMetadata?.participants)
            ...
            : Object.values(groupMetadata?.participants || {})
        const adminList = participants.filter(member => member?.isAdmin || member?.isSuperAdmin)
        const adminJids = await Promise.all(adminList.flatMap(member => [
            member.jid,
            member.lid,
            member.phoneNumber,
        ]).filter(Boolean).map(jid => resolveDatabaseJid(sock, jid)))
        m.isAdmin = adminJids.includes(m.sender)
        const botJid = await resolveDatabaseJid(sock, sock.user?.id)
        m.isBotAdmin = adminJids.includes(botJid)
    }

    m.pushName = msg.pushName
    const ownerJids = await resolveOwnerJids(sock, Array.isArray(db?.data?.setting?.owner) ? db.data.setting.owner : [])
    m.isOwner = Boolean(m.sender && ownerJids.includes(m.sender))
    if (m.message) {
    ...
    const attachEdit = (sent) => {
        const reference = sent && typeof sent === 'object' ? sent : { id: sent }
        reference.edit = (nextText, editOptions = {}) => m.edit(reference, nextText, editOptions)
        return reference
    }
    ...
    m.reply = async (text, trs, options = {}) => {
        ...
        if (isBinary) {
            ...
            if (isMedia && typeof sock.sendMedia === 'function') {
                return attachEdit(await sock.sendMedia(m.from, media, m.raw, {
                    ...options,
                    mimetype: options.mimetype || detectedMime,
                    fileName: options.fileName || `output.${detected?.ext || 'bin'}`,
                }))
            }
            return attachEdit(await sock.message.send(
                m.from,
                {
                    type: 'document',
                    media,
    ...
    m.edit = async (reference, text, options = {}) => {
        const editKey = reference?.key || reference?.editKey || reference
        const key = editKey?.id
            ? {
                remoteJid: editKey.remoteJid || m.from,
                id: editKey.id,
                fromMe: true,
            }
            : null
        if (!key) throw new Error('Referensi pesan untuk edit tidak valid.')
        return m.reply(text, false, {
            ...options,
            editKey: key,
        })
    }
