...
const { fileTypeFromBuffer } = file_type
...
/* load file */
export async function loads(modulePath) {
    ...
    // ESM only: .mjs, .esm.js
    if (ext === ".mjs" || ext === ".esm.js") {
        try {
            const fileUrl = pathToFileURL(absPath).href;
            mod = await import(fileUrl);
            return mod;
        } catch (e) {
            console.error(`[LOADS] ESM import failed for: ${absPath} (${e.message})`);
            return null;
        }
    }
    // CommonJS only: .cjs
        ...
        try {
            ...
            console.error(`[LOADS] CommonJS require failed for: ${absPath} (${err.message})`);
            return null;
        }
    }
    // .js: try ESM first, then CJS if ESM fails
    else if (ext === ".js") {
        // Try ESM
        try {
            const fileUrl = pathToFileURL(absPath).href;
            ...
            // Try CJS only if ESM fails
            try {
                // Baca isi file .js sebagai string
                const code = await fs.promises.readFile(absPath, "utf8");
                // Bungkus dalam fungsi dan evaluasi sebagai CommonJS
                const { createRequire } = await import('module');
                const require = createRequire(import.meta.url);
                ...
                wrapped(require, module, module.exports, __filename, __dirname);
                return module.exports;
            } catch (err) {
                console.error(`[LOADS] Failed to load .js module as ESM or CommonJS: ${absPath} (ESM: ${e.message}, CJS: ${err.message})`);
                return null;
            }
        }
    } else {
        console.error(`[LOADS] Unsupported file extension: ${ext}`);
        return null;
    }
}
...
export async function tr(q, lang) {
    const translatedText = await translate(q, { to: lang });
...
export async function writeExif(media, metadata = {}, gif = false) {
    ...
    const json = {
        "sticker-pack-id": "https://github.com/Haliq0841/Chitanda",
        "sticker-pack-name": metadata?.wm || "Chitanda BOT",
